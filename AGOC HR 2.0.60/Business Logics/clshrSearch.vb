Public Class clshrSearch
    ...
    Public Sub LoadForm(ByVal oForm As SAPbouiCOM.Form)
        ...
        oForm.DataSources.UserDataSources.Add("LastActdt", SAPbouiCOM.BoDataType.dt_DATE)
        oApplication.Utilities.setUserDatabind(oForm, "47", "LastActdt")
    ...
    Private Sub FillCountry(ByVal sform As SAPbouiCOM.Form)
        ...
        oSlpRS.DoQuery(" select distinct(isnull(T0.U_Z_Citizen,'')) AS 'Code',T1.Name AS 'Name' from [@Z_HR_OCRAPP] T0 Join OCRY T1 on T0.U_Z_Citizen=T1.Code")
        oCombobox.ValidValues.Add("", "")
    ...
    Private Sub Gridbind(ByVal aForm As SAPbouiCOM.Form)
        Dim strqry, strQry1, strminexp, strmaxexp, strnational, strsex, strReqno, strEducation, strmajor, strposition, strMainSkill, LastActDate, LastActDate1 As String
        Dim strReqCondition, strExpcondition, strNaticondition, strsexcondition, strCondition, streducondition, strmjrCondition, strposcondition, strMainSkillCondition, strLastActDateCon As String
        Dim DtLastAct, DtLastAct1 As Date
        ...
        LastActDate = oApplication.Utilities.getEdittextvalue(aForm, "47")
        LastActDate1 = oApplication.Utilities.getEdittextvalue(aForm, "49")
        ...
        If LastActDate <> "" Then
            DtLastAct = oApplication.Utilities.GetDateTimeValue(LastActDate)
        End If

        If LastActDate1 <> "" Then
            DtLastAct1 = oApplication.Utilities.GetDateTimeValue(LastActDate1)
        End If
        If LastActDate <> "" Then
            strLastActDateCon = " ( T0.U_Z_LatActDt >='" & DtLastAct.ToString("yyyy-MM-dd") & "'"
        Else
            strLastActDateCon = "( 1=1"
        End If

        If LastActDate1 <> "" Then
            strLastActDateCon = strLastActDateCon & " and T0.U_Z_LatActDt <='" & DtLastAct1.ToString("yyyy-MM-dd") & "' )"
        Else
            strLastActDateCon = strLastActDateCon & " and 1=1 )"
        End If
        ...
        End If
        If strnational <> "" Then
            strNaticondition = "T0.U_Z_Citizen='" & strnational & "'"
        Else
        ...
        If strMainSkill <> "" Then
            strMainSkillCondition = "T1.U_Z_SkillName like '%" & strMainSkill & "%'"
        Else
            strMainSkillCondition = "1=1"
        End If

        '  strCondition = strReqCondition & " and " & strExpcondition & " and " & strNaticondition & " and " & strsexcondition
        strCondition = strReqCondition & " and " & strExpcondition & " and " & strNaticondition & " and " & strsexcondition & " and " & streducondition & " And " & strmjrCondition & " And " & strposcondition & " and " & strMainSkillCondition & " and " & strLastActDateCon
        ...
        strqry = "select distinct(''),T0.U_Z_RequestCode,T0.DocEntry,U_Z_FirstName,T0.U_Z_Dob,T0.U_Z_Mobile ,T0.U_Z_EmailId,T0.U_Z_YrExp,T0.CreateDate,"
        strqry = strqry & " U_Z_Skills from  [@Z_HR_OCRAPP] T0 Left join [@Z_HR_CRAPP2] T1 on T0.DocEntry=T1.DocEntry Left join [@Z_HR_CRAPP3] T2 on T0.DocEntry=T2.DocEntry Left join [@Z_HR_CRAPP4] T3 on T0.DocEntry=T3.DocEntry "
        ...
        oGrid.Columns.Item("CreateDate").TitleObject.Caption = "Applicant Date"
        oGrid.Columns.Item("CreateDate").Editable = False
        oGrid.Columns.Item("U_Z_Skills").TitleObject.Caption = "Social Security Number"
        ...
        Try
            ...
            For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
                ...
                If oCheckbox.IsChecked(intRow) Then
                    ...
                    stSQL1 = stSQL1 & " where  T0.DocEntry='" & oGrid.DataTable.GetValue("U_Z_RequestCode", intRow) & "'"
                    ...
                    oGeneralData1.SetProperty("U_Z_AppDate", oGrid.DataTable.GetValue("CreateDate", intRow))
