Public Class clshrMPRequest
    ...
    Private Sub BindExtEmpNo(ByVal aForm As SAPbouiCOM.Form, ByVal EmpNo As String)
        ...
        oRec.DoQuery("Select ""U_Z_EmpID"",""ExtEmpNo"" from OHEM where ""empID""='" & EmpNo & "'")
        If oRec.RecordCount > 0 Then
            If oRec.Fields.Item(0).Value <> "" Then
                oApplication.Utilities.setEdittextvalue(aForm, "141", oRec.Fields.Item(1).Value)
            Else
    ...
    Private Sub FillPosition1(ByVal sform As SAPbouiCOM.Form, ByVal empid As String)
        ...
        If oslpRec.RecordCount > 0 Then
            ...
            oSlpRS.DoQuery("Select ""U_Z_PosCode"",""U_Z_PosName""  from ""@Z_HR_OPOSIN"" where Isnull(U_Z_IsVacant,'Y')='Y' And ""U_Z_DeptCode"" in (Select ""Code"" from OUDP where ""U_Z_HOD""='" & empid & "')  order by ""DocEntry""")
        ...
        Else
            oSlpRS = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oSlpRS.DoQuery("Select ""U_Z_PosCode"",""U_Z_PosName""  from ""@Z_HR_OPOSIN"" where Isnull(U_Z_IsVacant,'Y')='Y'  order by ""DocEntry""")
            For intRow As Integer = 0 To oSlpRS.RecordCount - 1
                Try
                    oCombobox.ValidValues.Add(oSlpRS.Fields.Item(0).Value, oSlpRS.Fields.Item(1).Value)
                Catch ex As Exception
