...
#Region "Methods"
    Private Sub DataBind(ByVal aform As SAPbouiCOM.Form, ByVal EmpId As String)
        Try
            ...
            strQuery += " when 'A' then 'Approved' end as ""U_Z_Status"",""U_Z_AppRemarks"",isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS 'Current Approver',isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS 'Next Approver' from ""@Z_PAY_OLETRANS1"" T0 inner join ""@Z_PAY_LEAVE"" T1 on T0.""U_Z_TrnsCode""=T1.""Code"" where ""U_Z_EMPID""='" & EmpId & "' and ""U_Z_TransType""='L' order by T0.""Code"" Desc"
    ...
    Private Sub FillLeaveType(ByVal sform As SAPbouiCOM.Form)
        ...
        Try
            ...
            If oRecS.Fields.Item(0).Value = "" Then
                strSQL = "Select ""Code"",""Name"" from ""@Z_PAY_LEAVE"" order by ""U_Z_Sort"""
            Else
