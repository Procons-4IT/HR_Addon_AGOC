...
Public Class clshrExitInterview
    ...
    Public Sub LoadForm(ByVal oForm As SAPbouiCOM.Form)
        ...
        oApplication.Utilities.setEdittextvalue(oForm, "11", Now.Date.ToString("yyyyMMdd"))
        ...
        Try
            ...
            If oCombobox.Selected.Value = "E" Then
                If oApplication.Utilities.getEdittextvalue(aForm, "39") = "" Then
                    oApplication.Utilities.Message("Last working date is missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return False
                    'ElseIf oApplication.SBO_Application.MessageBox("Are you sure.do you want to close this document?", , "Yes", "No") = 2 Then
                    '    Return False
                End If
            End If
            Return True
    ...
    Private Sub UpdateAttachment(ByVal aForm As SAPbouiCOM.Form)
        ...
        End Try
    End Sub
    Private Sub LoadFiles(ByVal aform As SAPbouiCOM.Form, ByVal Rows As Integer, ByVal ItemId As String, ByVal ColId As String)
        oMatrix = aform.Items.Item(ItemId).Specific
        For intRow As Integer = 1 To oMatrix.RowCount
            'If oMatrix.IsRowSelected(intRow) Then
            Dim strFilename, strFilePath As String
            strFilename = oMatrix.Columns.Item(ColId).Cells.Item(Rows).Specific.value
            Dim Filename As String = Path.GetFileName(strFilename)
            strFilePath = oMatrix.Columns.Item(ColId).Cells.Item(Rows).Specific.value
...
#Region "AddtoUDT"
    Private Function AddtoUDT1(ByVal aform As SAPbouiCOM.Form) As Boolean
        ...
        strTermination = oApplication.Utilities.getEdittextvalue(aform, "39")
        ...
        If strTermination <> "" Then
            ...
            Dim oTest As SAPbobsCOM.Recordset
            oTest = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oTest.DoQuery("Select * from [@Z_PAY_OLETRANS] where U_Z_EmpId='" & oApplication.Utilities.getEdittextvalue(aform, "13") & "' and U_Z_IsTerm='Y'")
...
#End Region
    ...
    Private Sub AddOffCycleTable(ByVal aCode As String)
        ...
        Else
            strTermination = oApplication.Utilities.getEdittextvalue(oForm, "39")
            If strTermination <> "" Then
                DtTermination = oApplication.Utilities.GetDateTimeValue(strTermination)
            End If
            strCode = aCode
                        ...
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                If pVal.ItemUID = "1" And oForm.Mode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE Then
                                    'If oApplication.SBO_Application.MessageBox("Are you sure.do you want to close this document?", , "Yes", "No") = 2 Then
                                    '    Exit Sub
                                    'Else
                                    If Validation(oForm) = False Then
                                        BubbleEvent = False
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_MATRIX_LINK_PRESSED
                                ...
                                If pVal.ItemUID = "48" And pVal.ColUID = "V_3" Then
                                    LoadFiles(oForm, pVal.Row, pVal.ItemUID, pVal.ColUID)
                                    BubbleEvent = False
                                    Exit Sub
                                End If
                        End Select
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_DOUBLE_CLICK
                                ...
                                If pVal.ItemUID = "48" And pVal.ColUID = "V_3" Then
                                    ...
                                    Else
                                        ...
                                        CopyAttachment(strSelectedFolderPath)
                                    End If
                                End If
                                ...
                                Select Case pVal.ItemUID
                                    Case "3"
                                        ...
                                        If oForm.PaneLevel = 3 Then
                                            ...
                                            If oForm.Items.Item("53").Enabled = False Then
                                                oForm.Items.Item("53").Enabled = True
                                            End If
                                            ...
                                            Else
                                                ...
                                                If oForm.Items.Item("53").Enabled = True Then
    ...
    Public Sub FormDataEvent(ByRef BusinessObjectInfo As SAPbouiCOM.BusinessObjectInfo, ByRef BubbleEvent As Boolean)
        Try
            If BusinessObjectInfo.BeforeAction = False And BusinessObjectInfo.ActionSuccess = True And (BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_UPDATE) Then
                ...
                stXML = stXML.Replace("<?xml version=""1.0"" encoding=""UTF-16"" ?><Exit FormParams><DocEntry>", "")
                stXML = stXML.Replace("</DocEntry></Exit FormParams>", "")
                Dim otest As SAPbobsCOM.Recordset
                otest = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                If stXML <> "" Then
                    UpdateAttachment(oForm)
