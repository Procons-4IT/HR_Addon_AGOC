            ...
            If oProcesses.Length <> 0 Then
                For i As Integer = 0 To oProcesses.Length - 1
                    ...
                    If oDialogBox.ShowDialog(MyWindow) = DialogResult.OK Then
                        ...
                        strSelectedFilepath = strFilepath
...
#Region "Delete Row"
    ...
    Private Sub LoadFiles1(aFileName As String)
...
#End Region
    ...
    Private Sub reDrawForm(ByVal oForm As SAPbouiCOM.Form)
        Try
            ...
            oForm.Items.Item("1000007").Height = oForm.Height - 255
...
ByVal Calendar As String, ByVal DateLangCulture As String) As String
        ...
        DTFormat.ShortDatePattern = "ddMMyyyy"
...
#End Region
    ...
    Private Function ReadExcelData_New(ByVal aForm As SAPbouiCOM.Form, ByVal strID As String) As Boolean
        ...
        Dim strPath As String = CType(aForm.Items.Item(strID).Specific, SAPbouiCOM.EditText).Value
            ...
            For i As Integer = 1 To w.Sheets.Count
                ...
                If array IsNot Nothing Then
                    ...
                    For j As Integer = 1 To 2
                        ...
                        Try
                            If j > 1 And array(j, 1).ToString <> "" Then
                                '  MsgBox(array(j, 9).ToString)
                                oApplication.Utilities.setEdittextvalue(aForm, "177", array(j, 1))
                                Dim dtdate As Date = CDate(array(j, 2).ToString)
                                oApplication.Utilities.setEdittextvalue(aForm, "183", dtdate.ToString("yyyyMMdd"))
                                oApplication.Utilities.setEdittextvalue(aForm, "78", array(j, 3))
                                oApplication.Utilities.setEdittextvalue(aForm, "8", array(j, 4))
                                oApplication.Utilities.setEdittextvalue(aForm, "10", array(j, 5))
                                oApplication.Utilities.setEdittextvalue(aForm, "1000002", array(j, 6))

                                oApplication.Utilities.setEdittextvalue(aForm, "179", array(j, 7))
                                oApplication.Utilities.setEdittextvalue(aForm, "181", array(j, 8))
                                Try
                                    oApplication.Utilities.setEdittextvalue(aForm, "1000004", array(j, 9))
                                Catch ex As Exception

                                End Try

                                oCombobox = aForm.Items.Item("98").Specific
                                Try
                                    oCombobox.Select(array(j, 10).ToString, SAPbouiCOM.BoSearchKey.psk_ByDescription)
                                Catch ex As Exception

                                End Try

                                oApplication.Utilities.setEdittextvalue(aForm, "169", array(j, 11))
                                oCombobox = aForm.Items.Item("94").Specific
                                Try
                                    oCombobox.Select(array(j, 12).ToString, SAPbouiCOM.BoSearchKey.psk_ByDescription)
                                Catch ex As Exception

                                End Try
                                oApplication.Utilities.setEdittextvalue(aForm, "47", array(j, 13))
                                Dim expSalary As String = array(j, 14)
                                Try
                                    oApplication.Utilities.setEdittextvalue(aForm, "160", expSalary.Replace("$", ""))
                                Catch ex As Exception
                                    oApplication.Utilities.setEdittextvalue(aForm, "160", 0)
                                End Try


                                oApplication.Utilities.setEdittextvalue(aForm, "1000008", array(j, 23))
                                oApplication.Utilities.setEdittextvalue(aForm, "1000009", array(j, 24))
                                oApplication.Utilities.setEdittextvalue(aForm, "189", array(j, 20))

                                'Educational details
                                If array(j, 15).ToString <> "" Then
                                    oMatrix = aForm.Items.Item("60").Specific
                                    oMatrix.AddRow()
                                    oApplication.Utilities.SetMatrixValues(oMatrix, "V_5", oMatrix.RowCount, array(j, 18))
                                    oApplication.Utilities.SetMatrixValues(oMatrix, "V_4", oMatrix.RowCount, array(j, 17))
                                    oApplication.Utilities.SetMatrixValues(oMatrix, "V_7", oMatrix.RowCount, array(j, 16))
                                    oApplication.Utilities.SetMatrixValues(oMatrix, "V_6", oMatrix.RowCount, array(j, 15))

                                    'Dim dtdate1 As Date = CDate(array(j, 16).ToString)
                                    'Try
                                    '    oApplication.Utilities.SetMatrixValues(oMatrix, "V_1", oMatrix.RowCount, dtdate1.ToString("yyyyMMdd"))
                                    'Catch ex As Exception
                                    'End Try
                                    oCombobox = oMatrix.Columns.Item("V_2").Cells.Item(oMatrix.RowCount).Specific
                                    Try
                                        oCombobox.Select(array(j, 15), SAPbouiCOM.BoSearchKey.psk_ByDescription)
                                    Catch ex As Exception
                                    End Try

                                End If
                                'Skill Sets
                                If array(j, 19).ToString <> "" Then
                                    oMatrix = aForm.Items.Item("58").Specific
                                    oMatrix.AddRow()
                                    oApplication.Utilities.SetMatrixValues(oMatrix, "V_0", oMatrix.RowCount, array(j, 19))
                                    'oApplication.Utilities.SetMatrixValues(oMatrix, "V_4", oMatrix.RowCount, array(j, 17))
                                End If
                                If array(j, 21).ToString <> "" Then
                                    oMatrix = aForm.Items.Item("61").Specific
                                    oMatrix.AddRow()
                                    oApplication.Utilities.SetMatrixValues(oMatrix, "V_2", oMatrix.RowCount, array(j, 21))
                                    oApplication.Utilities.SetMatrixValues(oMatrix, "V_3", oMatrix.RowCount, array(j, 22))
                                End If
                            End If
                        Catch ex As Exception
                            oApplication.Utilities.Message(ex.Message, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    ...
                    Case False
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_GOT_FOCUS
                                oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                If pVal.ItemUID = "32" And pVal.CharPressed = 9 Then
                                    Dim stdate As Date = oApplication.Utilities.GetDateTimeValue(oApplication.Utilities.getEdittextvalue(oForm, "32"))
                                    Dim Hiriji As String = ConvertDateCalendar(stdate, "Hijri", "en-")

                                    Dim hj As New CultureInfo("ar-SA")
                                    Dim hjyr, hjmo, hjday As Integer, gdate As DateTime
                                    gdate = stdate  'or any other date
                                    ...
                                    Hiriji = hjyr & hj.DateTimeFormat.GetMonthName(hjmo) & hjday

                                    '  Hiriji = hjyr.ToString() & hj.DateTimeFormat.GetMonthName(hjmo).ToString & hjday.ToString
                                    ' Hiriji = hjday.ToString & hj.DateTimeFormat.GetMonthName(hjmo).ToString & hjyr.ToString



                                    oApplication.Utilities.setEdittextvalue(oForm, "173", Hiriji)
                                    Dim age As Integer
                                    age = Today.Year - stdate.Year
                                    If (stdate > Today.AddYears(-age)) Then age -= 1
                                    oApplication.Utilities.setEdittextvalue(oForm, "169", age)
                                End If
                            Case SAPbouiCOM.BoEventTypes.et_KEY_DOWN
                                oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                If pVal.ItemUID = "32" And pVal.CharPressed = 9 Then
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                Select Case pVal.ItemUID
                                    Case "171"
                                        Dim stdate As Date = oApplication.Utilities.GetDateTimeValue(oApplication.Utilities.getEdittextvalue(oForm, "171"))
                                        Dim Hiriji As String = ConvertDateCalendar(stdate, "Hijri", "en-")
                                        oApplication.Utilities.setEdittextvalue(oForm, "173", Hiriji)
                                        Dim age As Integer
                                        age = Today.Year - stdate.Year
                                        ...
                                        oApplication.Utilities.setEdittextvalue(oForm, "169", age)
                                    Case "186"
                                        Dim strPath As String = oApplication.Utilities.getEdittextvalue(oForm, "185")
                                        FileOpen()
                                        If strFilepath = "" Then
                                            oApplication.Utilities.Message("Please Select a File", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                            BubbleEvent = False
                                        Else
                                            oApplication.Utilities.setEdittextvalue(oForm, "185", strFilepath)
                                            '    CopyAttachment(strFilepath)
                                        End If
                                    Case "187"
                                        ReadExcelData_New(oForm, "185")
                                    Case "167"
                                        ...
                                        Else
                                            oApplication.Utilities.Message("Interview summary not found...", SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                                        End If
                                    Case "1"
                                        If oForm.Mode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Then
                                            AddMode(oForm)
                                    ...
                                    Case "63"
                                        ...
                                        If strSelectedFilepath <> "" Then
                                            ...
                                            Try
                                                ...
                                                CopyAttachment(strSelectedFilepath)
...
#End Region
    ...
    Private Sub EnableDisable(ByVal aForm As SAPbouiCOM.Form, ByVal strStatus As String)
        If strStatus = "H" Then
            ...
            aForm.Items.Item("171").Enabled = False
            aForm.Items.Item("175").Enabled = False
            aForm.Items.Item("183").Enabled = False
        Else
            ...
            aForm.Items.Item("171").Enabled = True
            aForm.Items.Item("175").Enabled = True
            aForm.Items.Item("183").Enabled = True
        End If
    End Sub
    ...
    Public Sub FormDataEvent(ByRef BusinessObjectInfo As SAPbouiCOM.BusinessObjectInfo, ByRef BubbleEvent As Boolean)
        Try
            If BusinessObjectInfo.BeforeAction = False And BusinessObjectInfo.ActionSuccess = True And (BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_LOAD) Then
                ...
                Dim Empid As String = oApplication.Utilities.getEdittextvalue(oForm, "71")
                If strHired <> "H" Then
                    oForm.Items.Item("8").Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                    oForm.Items.Item("4").Enabled = False
                    oForm.Items.Item("6").Enabled = False
                    EnableDisable(oForm, strHired)
                    oApplication.Utilities.setEdittextvalue(oForm, "185", "")
                Else
                    EnableDisable(oForm, strHired)
                    oForm.Items.Item("4").Enabled = False
                    oForm.Items.Item("6").Enabled = False
