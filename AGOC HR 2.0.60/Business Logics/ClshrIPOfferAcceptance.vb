...
#Region "LoadForm"
    ...
    Public Sub LoadForm(ByVal sFunction As String, ByVal strRQType As String)
        ...
        strHeaderQry = ""
        strHeaderQry = "Select T0.""DocEntry"",T0.""U_Z_HRAppID"" as ""App/ID"",T0.""U_Z_HRAppName"" as ""App/Name"",T0.""U_Z_DeptName"" as ""Department Name"",T1.U_Z_PosName AS 'PosName',T0.""U_Z_JobPosi"" as ""Position Name"", "
        ...
        strHeaderQry += " Union All Select T0.""DocEntry"",T0.""U_Z_HRAppID"" as ""App/ID"",T0.""U_Z_HRAppName"" as ""App/Name"",T0.""U_Z_DeptName"" as ""Department Name"",T1.U_Z_PosName AS 'PosName',T0.""U_Z_JobPosi"" as ""Position Name"", "
        strHeaderQry += " T0.""U_Z_ReqNo"" as ""Req No"",T1.""U_Z_AppStatus"" As ""Request Status"",T0.""U_Z_Email"" as ""EMail"",T0.""U_Z_Mobile"" as ""Mobile"",T0.""U_Z_ApplStatus"" as ""App Status"", T0.""U_Z_Skills"" as ""Skills"",T0.""U_Z_YrExp"" as ""YOE"","
        strHeaderQry += " Case T0.U_Z_AppRequired when 'Y' then 'Yes' else 'No' End as  'Approval Required', case T0.""U_Z_IPLUSta"" when 'S' then 'Selected' when 'R' then 'Rejected' when 'S' then 'Selected' else 'Pending' end as ""LM Status"",case T0.""U_Z_IPHODSta"" when 'S' "
        strHeaderQry += " then 'Selected' when 'R'then 'Rejected' when 'S' then 'Selected' else 'Pending' end as ""HOD Status"",case T0.""U_Z_IPHRSta"" when 'S' then 'Selected' when 'R'then "
        'strHeaderQry += " 'Rejected' when 'S' then 'Selected' else 'Pending' end as 'HR Status',U_Z_Finished As 'Work Flow Status' "
...
#Region "For HR"
    ...
    Private Sub offerDetails(ByVal strDocEntry As String)
        ...
        strDetailQry = ""
        strDetailQry = "Select ""LineId"",""U_Z_Basic"",""U_Z_Benifit"",""U_Z_TotalSalary"",""U_Z_Attachment"",""U_Z_Status"",""U_Z_JoinDate"",""U_Z_RejReason"",""U_Z_Remarks"",""U_Z_RefNo"" From ""@Z_HR_OHEM3"" Where ""DocEntry"" = '" & strDocEntry & "'"
        oOfferGrid.DataTable.ExecuteQuery(strDetailQry)

        oOfferGrid.SelectionMode = SAPbouiCOM.BoMatrixSelect.ms_Single
        oOfferGrid.Columns.Item("LineId").Visible = False
        oOfferGrid.Columns.Item("U_Z_Basic").TitleObject.Caption = "Offered Basic"
        oOfferGrid.Columns.Item("U_Z_Basic").Editable = False
        oOfferGrid.Columns.Item("U_Z_Benifit").TitleObject.Caption = "Benifits"
        oOfferGrid.Columns.Item("U_Z_Benifit").Editable = False
        oOfferGrid.Columns.Item("U_Z_TotalSalary").TitleObject.Caption = "Total Salary"
        oOfferGrid.Columns.Item("U_Z_TotalSalary").Editable = False
        ...
        oOfferGrid.Columns.Item("U_Z_RefNo").Editable = False
    ...
    Private Function oFferAcceptance_DIAPI(ByVal DocNo As String) As Boolean
        ...
        Try
            ...
            For i As Integer = 0 To oOfferGrid.DataTable.Rows.Count - 1
                ...
                oChildData.SetProperty("U_Z_TotalSalary", oOfferGrid.DataTable.GetValue("U_Z_TotalSalary", i))
                oChildData.SetProperty("U_Z_RefNo", oOfferGrid.DataTable.GetValue("U_Z_RefNo", i))
...
#Region "HR Status & Comments Update"
    ...
    Private Sub HR_SC_Update_Finished(ByVal DocNo As String, ByVal strAPPID As String, ByVal Status As String, ByVal Comments As String, ByVal aBasic As Double, ByVal stJDate As String, ByVal dtJDate As Date, ByVal aBenifit As Double, ByVal aRef As String)
        Dim oRS As SAPbobsCOM.Recordset
        oRS = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        'Candidate Job Offered
        strQuery = "Update ""@Z_HR_OHEM1"" set ""U_Z_OfferStatus"" = '" & Status & "' where ""DocEntry"" = '" & DocNo & "'"
        ...
        ElseIf Status = "A" Then
            ...
            oRS.DoQuery(strQuery)
            If stJDate = "" Then
                strQuery = "Update ""@Z_HR_OCRAPP"" set ""U_Z_OffBasic"" = '" & aBasic & "',""U_Z_Benifit""='" & aBenifit & "',""U_Z_RefNo""='" & aRef & "', ""U_Z_Status"" = 'A' where ""DocEntry"" = '" & strAPPID & "'"
            Else
                Dim strmonth, strdate, stryear As String
                Dim Jdate As Date '= oApplication.Utilities.GetDateTimeValue(dtJDate)
                strQuery = "Update ""@Z_HR_OCRAPP"" set ""U_Z_OffBasic"" = '" & aBasic & "',""U_Z_Benifit""='" & aBenifit & "',""U_Z_RefNo""='" & aRef & "',""U_Z_JoinDate"" = '" & dtJDate.ToString("yyyy-MM-dd") & "', ""U_Z_Status"" = 'A' where ""DocEntry"" = '" & strAPPID & "'"
            End If
            oRS.DoQuery(strQuery)
        ElseIf Status = "J" Then
            strQuery = "Update ""@Z_HR_OHEM1"" set ""U_Z_Finished"" = 'Y' where ""DocEntry"" = '" & DocNo & "'"
                                        ...
                                        Else
                                            For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
                                                If oGrid.Rows.IsSelected(intRow) Then
                                                    ...
                                                    Dim strOStatus As String = oOfferGrid.DataTable.GetValue("U_Z_Status", oOfferGrid.DataTable.Rows.Count - 1)
                                                    Dim dblBasic As Double = oOfferGrid.DataTable.GetValue("U_Z_Basic", oOfferGrid.DataTable.Rows.Count - 1)
                                                    Dim dblBenifit As Double = oOfferGrid.DataTable.GetValue("U_Z_Benifit", oOfferGrid.DataTable.Rows.Count - 1)
                                                    Dim strAllRef As String = oOfferGrid.DataTable.GetValue("U_Z_RefNo", oOfferGrid.DataTable.Rows.Count - 1)
                                                    Dim dtJoinDt As String = oOfferGrid.DataTable.GetValue("U_Z_JoinDate", oOfferGrid.DataTable.Rows.Count - 1)

                                                    oFferAcceptance_DIAPI(strDocEntry)
                                                    HR_SC_Update(strDocEntry, strAppID, strOStatus, strComment)
                                                    If dtJoinDt = "" Then
                                                        HR_SC_Update_Finished(strDocEntry, strAppID, strOStatus, strComment, dblBasic, "", Now.Date, dblBenifit, strAllRef)
                                                    Else
                                                        Dim dat As Date = oOfferGrid.DataTable.GetValue("U_Z_JoinDate", oOfferGrid.DataTable.Rows.Count - 1)
                                                        HR_SC_Update_Finished(strDocEntry, strAppID, strOStatus, strComment, dblBasic, dtJoinDt, dat, dblBenifit, strAllRef)
                                                    End If
                                                End If
                                            Next
                                            ' oForm.Close()
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_DOUBLE_CLICK
                                If pVal.ItemUID = "6" And pVal.ColUID = "U_Z_Attachment" Then
                                    ...
                                    Else
                                        ...
                                        CopyAttachment(strFilepath)
                                    End If
                                ElseIf pVal.ItemUID = "6" And (pVal.ColUID = "U_Z_TotalSalary" Or pVal.ColUID = "U_Z_Basic" Or pVal.ColUID = "U_Z_Benifit") Then
                                    oOfferGrid = oForm.Items.Item("1").Specific
                                    oGrid = oForm.Items.Item("6").Specific
                                    Dim RefNo As String = oGrid.DataTable.GetValue("U_Z_RefNo", pVal.Row)
                                    If oOfferGrid.Rows.Count > 0 Then
                                        For intRow As Integer = 0 To oOfferGrid.DataTable.Rows.Count - 1
                                            If oOfferGrid.Rows.IsSelected(intRow) Then
                                                Dim AppID As String = oOfferGrid.DataTable.GetValue("App/ID", intRow)
                                                Dim AppName As String = oOfferGrid.DataTable.GetValue("App/Name", intRow)
                                                Dim Basic As String = oGrid.DataTable.GetValue("U_Z_Basic", pVal.Row)
                                                Dim DocEntry As String = oOfferGrid.DataTable.GetValue("DocEntry", intRow)
                                                Dim LineNo As String = pVal.Row
                                                ' AddtoUDT(oForm, RefNo, DocEntry, LineNo, AppID, AppName, Basic)
                                                AddtoUDT_Allowance(oForm, RefNo, DocEntry, LineNo, AppID, AppName, Basic)
                                                oForm.Refresh()
                                                oOfferGrid.Columns.Item("RowsHeader").Click(pVal.Row, False, False)
                                                Dim RefNo1 As String = oGrid.DataTable.GetValue("U_Z_RefNo", pVal.Row)
                                                Dim LineNo1 As String = pVal.Row
                                                Dim oObj As New clshrOfferAllowance()
                                                frmOfferAcceptance = oForm
                                                oObj.LoadForm(AppID, AppName, DocEntry, RefNo1, Basic, LineNo1)
                                                Exit For
                                            End If
                                        Next
                                    End If
...
#End Region
    ...
    Private Sub AddtoUDT_Allowance(ByVal aForm As SAPbouiCOM.Form, ByVal RefNo As String, ByVal DocEntry As String, ByVal LineNo As String, ByVal AppId As String, ByVal AppName As String, ByVal Basic As String)
        Try
            oOfferGrid = oForm.Items.Item("6").Specific
            ...
            oUserTable = oApplication.Company.UserTables.Item("Z_HR_OEALL")
            ouserTableLines = oApplication.Company.UserTables.Item("Z_HR_EALL1")
            'strQuery = "select U_Z_RefNo,Code from [@Z_HR_OEALL] where U_Z_RefNo='" & DocEntry & "' and U_Z_LineNo='" & LineNo & "'"
            'oRecSet.DoQuery(strQuery)
            'If oRecSet.RecordCount > 0 Then
            '    RefNo = oRecSet.Fields.Item("Code").Value
            'End If
            If RefNo = "" Then
                strCode = oApplication.Utilities.getMaxCode("@Z_HR_OEALL", "Code")
                oUserTable.Code = strCode
                oUserTable.Name = strCode
                oUserTable.UserFields.Fields.Item("U_Z_AppID").Value = AppId
                oUserTable.UserFields.Fields.Item("U_Z_AppName").Value = AppName
                oUserTable.UserFields.Fields.Item("U_Z_Basic").Value = Basic
                oUserTable.UserFields.Fields.Item("U_Z_RefNo").Value = DocEntry
                oUserTable.UserFields.Fields.Item("U_Z_LineNo").Value = LineNo
                If oUserTable.Add <> 0 Then
                    oApplication.Utilities.Message(oApplication.Company.GetLastErrorDescription, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Else
                    oGrid = aForm.Items.Item("6").Specific
                    oGrid.DataTable.SetValue("U_Z_RefNo", LineNo, strCode)
                End If
            ElseIf oUserTable.GetByKey(RefNo) Then
                'oUserTable.Code = RefNo
                'oUserTable.Name = RefNo
                'oUserTable.UserFields.Fields.Item("U_Z_AppID").Value = AppId
                'oUserTable.UserFields.Fields.Item("U_Z_AppName").Value = AppName
                'oUserTable.UserFields.Fields.Item("U_Z_Basic").Value = Basic
                'oUserTable.UserFields.Fields.Item("U_Z_RefNo").Value = DocEntry
        ...
        End Try
    End Sub
    Private Sub AddtoUDT(ByVal aForm As SAPbouiCOM.Form, ByVal RefNo As String, ByVal DocEntry As String, ByVal LineNo As String, ByVal AppId As String, ByVal AppName As String, ByVal Basic As String)
        Try
            oOfferGrid = oForm.Items.Item("6").Specific
            Dim oRecSet As SAPbobsCOM.Recordset
            Dim dblAmount As Double = 0
            ...
            oGenData = oGenService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData)
            oGeneralDataParams = oGenService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
            oGeneralDataParams.SetProperty("DocEntry", Convert.ToInt32(DocEntry))
            oGenData = oGenService.GetByParams(oGeneralDataParams)
            oGenDataCollection = oGenData.Child("Z_HR_OHEM3")
            For i As Integer = 0 To oOfferGrid.DataTable.Rows.Count - 1
                If i > oGenDataCollection.Count - 1 Then
                ...
                Else
                    oChildData = oGenDataCollection.Item(i)
                End If
                oChildData.SetProperty("U_Z_RefNo", RefNo)
                oOfferGrid.DataTable.SetValue("U_Z_RefNo", i, RefNo)
            Next
            oGenService.Update(oGenData)
