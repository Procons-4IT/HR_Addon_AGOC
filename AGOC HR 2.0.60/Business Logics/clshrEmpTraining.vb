Public Class clshrEmpTraining
    ...
    Public Sub LoadForm(ByVal Empid As String, ByVal EmpName As String, Optional ByVal poscode As String = "", Optional ByVal posiname As String = "")
        ...
        ExpintTempID = oApplication.Utilities.CheckApprovalExists(HeaderDoctype.Train, Empid)
    ...
    Private Sub NewTrainingSummary(ByVal strempid As String)
        ...
        strqry = strqry & " end as U_Z_HRStatus,U_Z_HRRemarks,isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS 'Current Approver',isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS 'Next Approver'  from [@Z_HR_ONTREQ] T0 where U_Z_HREmpID='" & strempid & "'"
        oGrid.DataTable.ExecuteQuery(strqry)
    ...
    Private Sub Databind(ByVal empid As String, ByVal aForm As SAPbouiCOM.Form)
        ...
        Try
            ...
            strqry = strqry & " case U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' else 'Rejected' end as U_Z_AppStatus,isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS 'Current Approver',isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS 'Next Approver', case U_Z_Status when 'P' then 'Pending' when 'A' then 'Accepted' else 'Rejected' end as U_Z_Status,U_Z_Remarks,Code from [@Z_HR_TRIN1] T0 where U_Z_HREmpID='" & empid & "' "
            oGrid.DataTable.ExecuteQuery(strqry)
            oGrid.Columns.Item("U_Z_HREmpID").TitleObject.Caption = "Employee Id"
            oGrid.Columns.Item("U_Z_HREmpID").Visible = False
            oEditTextColumn = oGrid.Columns.Item("U_Z_HREmpID")
