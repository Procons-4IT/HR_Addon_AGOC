    ...
    Public Sub FormDataEvent(ByRef BusinessObjectInfo As SAPbouiCOM.BusinessObjectInfo, ByRef BubbleEvent As Boolean)
        Try
            ...
            If BusinessObjectInfo.BeforeAction = False And (BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_ADD Or BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_UPDATE) Then
                oForm = oApplication.SBO_Application.Forms.ActiveForm()
                Dim strdocnum, strMessage, strSql As String
                ...
                Dim otest, oRecSet, OtempSet As SAPbobsCOM.Recordset
                otest = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                oRecSet = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                OtempSet = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                If stXML <> "" Then
                    otest.DoQuery("select * from [@Z_HR_OEXFOM]  where DocEntry=" & stXML)
                    If otest.RecordCount > 0 Then
                        strMessage = "Employee exit form is initialized"
                        oApplication.Utilities.SendMailExitForm(strMessage, 0, otest.Fields.Item("U_Z_empID").Value)

                        strSql = "Select * from [@Z_HR_EXFORM4] where DocEntry=" & stXML
                        oRecSet.DoQuery(strSql)
                        If oRecSet.RecordCount > 0 Then
                            For intRow As Integer = 0 To oRecSet.RecordCount - 1
                                strMessage = "Fixed asset is initialized.Initialization code is :" & stXML & ".Fixed Asset code/Description is:" & oRecSet.Fields.Item("U_Z_ObjCode").Value & "/" & oRecSet.Fields.Item("U_Z_ObjName").Value & ". Need your approval."
                                oApplication.Utilities.SendMailExitForm(strMessage, 0, oRecSet.Fields.Item("U_Z_ResID").Value)
                                oRecSet.MoveNext()
                            Next
                        End If
                        strSql = "Select * from [@Z_HR_EXFORM1] where DocEntry=" & stXML
                        OtempSet.DoQuery(strSql)
                        If OtempSet.RecordCount > 0 Then
                            For intRow As Integer = 0 To OtempSet.RecordCount - 1
                                strMessage = "Exit form of responsibilities are initialized.Initialization code is :" & stXML & "Responsibility is :" & OtempSet.Fields.Item("U_Z_ResDesc").Value & ".Need your approval."
                                oApplication.Utilities.SendMailExitForm(strMessage, 0, OtempSet.Fields.Item("U_Z_ResID").Value)
                                OtempSet.MoveNext()
                            Next
                        End If
                    End If
