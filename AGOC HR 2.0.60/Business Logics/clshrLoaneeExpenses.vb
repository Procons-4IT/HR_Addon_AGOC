Public Class clshrLoaneeExpenses
    Inherits clsBase
    ...
    End Sub
    Public Sub LoadForm()
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_hr_LExpenses) = False Then
            oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Exit Sub
        End If
        oForm = oApplication.Utilities.LoadForm(xml_hr_LExpenses, frm_hr_LExpenses)
...
#Region "DataBind"
    ...
    Public Sub BindData(ByVal objform As SAPbouiCOM.Form)
        ...
        Try
            ...
            otestrs.DoQuery("Update ""@Z_HR_LEXPANCES"" set Name=' '  where ""Name"" Like '%_XD'")
            oMatrix = objform.Items.Item("3").Specific
            oDBDataSrc = objform.DataSources.DBDataSources.Add("@Z_HR_LEXPANCES")
            Try
                oDBDataSrc.Query()
            Catch ex As Exception
...
#Region "Enable Matrix After Update"
    ...
    Private Sub EnblMatrixAfterUpdate(ByVal objApplication As SAPbouiCOM.Application, ByVal ocompany As SAPbobsCOM.Company, ByVal oForm As SAPbouiCOM.Form)
        ...
        Try
            ...
            End If
            oUserTable = ocompany.UserTables.Item("Z_HR_LEXPANCES")
            oDBDSource = oForm.DataSources.DBDataSources.Item("@Z_HR_LEXPANCES")
            oMatrix.FlushToDataSource()
            For i = 0 To oDBDSource.Size - 1
                oGeneralService = oCompanyService.GetGeneralService("Z_HR_LEXPANCES")
                oGeneralData = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData)
                strCode = oDBDSource.GetValue("Code", i).Trim
                otestRs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                oGeneralParams = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
                otestRs.DoQuery("Select * from [@Z_HR_LEXPANCES] where Code='" & strCode & "'")
                If otestRs.RecordCount > 0 Then
                    ...
                    oGeneralData.SetProperty("U_Z_Amount", oDBDSource.GetValue("U_Z_Amount", i))
                    oGeneralData.SetProperty("U_Z_Currency", oDBDSource.GetValue("U_Z_Currency", i))
                    ...
                    Dim strstatus1 As String = oDBDSource.GetValue("U_Z_WeekEnd", i)
                    oGeneralData.SetProperty("U_Z_WeekEnd", strstatus1.Trim)
                    Dim strOver As String = oDBDSource.GetValue("U_Z_OverLap", i)
                    oGeneralData.SetProperty("U_Z_OverLap", strOver.Trim())
                    Dim strExpType As String = oDBDSource.GetValue("U_Z_ExpType", i)
                    oGeneralData.SetProperty("U_Z_ExpType", strExpType.Trim)
                    Dim strPerdim As String = oDBDSource.GetValue("U_Z_Perdim", i)
                    oGeneralData.SetProperty("U_Z_Perdim", strPerdim.Trim)
...
#Region "Insert Code and Doc Entry"
    ...
    Public Sub InsertCodeAndDocEntry(ByVal aForm As SAPbouiCOM.Form)
        ...
        Try
            objForm = aForm
            objForm.Freeze(True)
            oDBDSource = objForm.DataSources.DBDataSources.Item("@Z_HR_LEXPANCES")
            objMatrix = objForm.Items.Item("3").Specific
            objMatrix.FlushToDataSource()
            Dim strCode, strDocEntry As String
            strCode = oApplication.Utilities.getMaxCode("@Z_HR_LEXPANCES", "Code")
            strDocEntry = oApplication.Utilities.getMaxCode("@Z_HR_LEXPANCES", "DocEntry")
        ...
        Try
            ...
            For intRow As Integer = aRow To aRow
                If oMatrix.IsRowSelected(intRow) Then
                    ...
                    oGeneralService = oCompanyService.GetGeneralService("Z_HR_LEXPANCES")
                    oGeneralParams = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
                    otestRs.DoQuery("Update ""@Z_HR_LEXPANCES"" set ""Name""=isnull(""Name"",'') + '_XD' where ""Code""='" & strCode & "'")
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_hr_LExpenses Then
                Select Case pVal.BeforeAction
                    ...
                    Case False
                        If ((pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED) And (pVal.ItemUID = "1")) Then
                            ...
                            Dim otestrs As SAPbobsCOM.Recordset
                            otestrs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                            otestrs.DoQuery("Delete from ""@Z_HR_LEXPANCES"" where ""Name"" Like '%_XD'")
                            objForm = oForm
                            objMatrix = objForm.Items.Item("3").Specific
                            objMatrix.AddRow()
                            ...
                            oCheckBox = objMatrix.Columns.Item("V_8").Cells.Item(objMatrix.RowCount).Specific
                            oCheckBox.Checked = False ' ("", SAPbouiCOM.BoSearchKey.psk_ByValue)
                            ...
                            Try
                                ...
                                If (oCFLEvento.BeforeAction = False) Then
                                    ...
                                    If pVal.ItemUID = "3" And pVal.ColUID = "V_17" Then
                                        ...
                                        objMatrix = oForm.Items.Item("3").Specific
                                        oApplication.Utilities.SetMatrixValues(objMatrix, "V_17", pVal.Row, val)
                                    End If
                                    oForm.Freeze(False)
                                End If
                            Catch ex As Exception
