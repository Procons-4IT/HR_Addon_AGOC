    ...
    Private Sub Databind(ByVal strempid As String)
        ...
        If oForm.Title = "Employee Travel Request" Or oForm.Title = "Travel Request Approval" Then
            If strempid = "" And oForm.Title = "Travel Request Approval" Then
                ...
                strqry = strqry & " when 'CA' then 'Claim Approved' when 'CJ' then 'Claim Rejected' else 'Closed' end as U_Z_Status, DocEntry,U_Z_DocDate,U_Z_EmpId,U_Z_EmpName,U_Z_DeptName,U_Z_PosName,U_Z_TraName,U_Z_TraStLoc,U_Z_TraEdLoc,U_Z_TraStDate,U_Z_TraEndDate,isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS 'Current Approver',isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS 'Next Approver' "
                strqry = strqry & " from [@Z_HR_OTRAREQ] T0 where U_Z_EmpId in (" & stremp & ") "
                oGrid.DataTable.ExecuteQuery(strqry)
                FormatGrid1(oGrid, "Travel")
            Else
                strqry = "select case U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end as U_Z_AppStatus,  case U_Z_Status when 'O' then 'Open' when 'RA' then 'Request Approved' when 'RR' then 'Request Rejected' when 'CR' then 'Claim Received'"
                strqry = strqry & " when 'CA' then 'Claim Approved' when 'CJ' then 'Claim Rejected' else 'Closed' end as U_Z_Status, DocEntry,U_Z_DocDate,U_Z_EmpId,U_Z_EmpName,U_Z_DeptName,U_Z_PosName,U_Z_TraName,U_Z_TraStLoc,U_Z_TraEdLoc,U_Z_TraStDate,U_Z_TraEndDate,isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS 'Current Approver',isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS 'Next Approver' "
                strqry = strqry & " from [@Z_HR_OTRAREQ] T0 where U_Z_EmpId in (" & strempid & ") "
                oGrid.DataTable.ExecuteQuery(strqry)
                FormatGrid1(oGrid, "Travel")
            End If
        End If
