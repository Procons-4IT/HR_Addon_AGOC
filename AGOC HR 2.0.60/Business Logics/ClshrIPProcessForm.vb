    ...
    Public Sub LoadForm(ByVal sFunction As String, ByVal strRQType As String)
        ...
        Select Case sFunction
            Case "IPLM", "IPLMU"
                ...
                strHeaderQry += " Where  T0.U_Z_ReqNo = " & strRQType & " And T0.U_Z_AppStatus = 'A' AND T0.U_Z_HRAppID IN (select DocEntry  from [@Z_HR_OCRAPP] where isnull(U_Z_EmpId,'') ='')"
...
#Region "LineManager DIAPI"
    Private Function LineManager_DIAPI(ByVal DocNo As String, ByVal strApplicantID As String, ByVal oDT As DataTable, ByVal iFunction As Integer, ByVal oHT As Hashtable) As Boolean
        Dim strAPPID, strRatPer As String
        Dim RetVal As Boolean
        Dim SchTime As Integer
        Dim dblRatPerc As Double = 0.0
        Dim dblTotalRatPerc As Double = 0.0
        RetVal = False
        ...
        Try
            ...
            For i As Integer = 0 To oDT.Rows.Count - 1
                ...
                If iFunction = 1 Then
                    ...
                    Dim intHours, intMin As Integer
                    intHours = CInt(st) / 100
                    intMin = CInt(st) - (intHours * 100)
                    st = st.Replace(":", "")
                    st = intHours.ToString("00")
                    st = st & intMin.ToString("00")


                    Dim myTime As DateTime = CDate(odr("SDate").ToString()) ' DateTime.Today.Date
                    '// Add the hours and minutes to the date
                    myTime = myTime.AddHours(st.Substring(0, 2))
                    myTime = myTime.AddMinutes(st.Substring(2, 2))

                    Try
                        Dim dtTimespance As TimeSpan
                        dtTimespance = New TimeSpan(st)
                        oChildData.SetProperty("U_Z_ScTime", myTime)
                    Catch ex As Exception

                    End Try

                ElseIf iFunction = 2 Then
                    oGenData.SetProperty("U_Z_IPLUSta", odr("IStatus").ToString())
                    oChildData.SetProperty("U_Z_HRAppID", strApplicantID)
                    ...
                    strRatPer = odr("RatngPer").ToString()
                    If strRatPer <> "" Then
                        dblRatPerc = oApplication.Utilities.getDocumentQuantity(strRatPer)
                    End If
                    If dblTotalRatPerc = 0.0 Then
                        dblTotalRatPerc = dblRatPerc
                    Else
                        dblTotalRatPerc = dblTotalRatPerc + dblRatPerc
                    End If
                End If
            Next
            ...
            If strFunction = "IPLM" Then
                If iFunction = 1 Then
                    sQuery = "Update [@Z_HR_OHEM1] Set U_Z_FinalApproval = 'N',U_Z_InvRating='" & dblTotalRatPerc & "' Where DocEntry = '" & DocNo & "'"
                    oRec.DoQuery(sQuery)
                End If
            ...
            Else
                If iFunction = 2 Then
                    If strInterviewsummerystatus <> "P" And strInterviewsummerystatus <> "-" Then
                        sQuery = "Update [@Z_HR_OHEM1] Set U_Z_FinalApproval = 'Y',U_Z_InvRating=" & dblTotalRatPerc & " Where isnull(U_Z_FinalApproval,'N')='N' and  DocEntry = '" & DocNo & "'"
                        oRec.DoQuery(sQuery)
                    End If
                End If
            End If
