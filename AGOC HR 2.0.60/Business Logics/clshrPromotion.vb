Public Class clshrPromotion
    ...
    Private Sub LoadForm(ByVal oForm As SAPbouiCOM.Form)
        ...
        oForm.DataSources.UserDataSources.Add("SalCode1", SAPbouiCOM.BoDataType.dt_SHORT_TEXT)
        oApplication.Utilities.setUserDatabind(oForm, "79", "SalCode1")
        'oEditText = oForm.Items.Item("79").Specific
    ...
    Private Sub AddChooseFromList(ByVal objForm As SAPbouiCOM.Form)
        Try
            ...
            'oCon = oCons.Add()
            'oCon.Alias = "U_Z_SalCode"
            'oCon.Operation = SAPbouiCOM.BoConditionOperation.co_BETWEEN
            'oCon.CondVal = oApplication.Utilities.getEdittextvalue(objForm, "90")
            'oCon.Relationship = SAPbouiCOM.BoConditionRelationship.cr_AND
            'oCon.CondVal = oApplication.Utilities.getEdittextvalue(objForm, "91")
    ...
    Private Sub Gridbind(ByVal strempid As String, ByVal aForm As SAPbouiCOM.Form)
        ...
        strqry += " case ""U_Z_AppStatus"" when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end as ""U_Z_AppStatus"",isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS 'Current Approver',isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS 'Next Approver'  from ""@Z_HR_HEM2"" T0 where ""U_Z_EmpId""='" & strempid & "' "
                    ...
                    Case False
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_KEY_DOWN
                                oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                If pVal.ItemUID = "79" And pVal.CharPressed = 9 Then
                                    oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                    ...
                                    clsChooseFromList.CFLChoice = "SalCode" 'oCombo.Selected.Value
                                    clsChooseFromList.choice = "Salary"
                                    clsChooseFromList.Documentchoice = "SalCode"
                                    'clsChooseFromList.ItemCode = oApplication.Utilities.getEdittextvalue(oForm, "13")
                                    'oComboColumn = oGrid.Columns.Item("U_Z_Dept")
                                    Try
                                        clsChooseFromList.BinDescrUID = oApplication.Utilities.getEdittextvalue(oForm, "13")
                                    Catch ex As Exception
                                ...
                                Select Case pVal.ItemUID
                                    ...
                                    Case "3"
                                        ...
                                        If oForm.PaneLevel = 3 Then
                                            ...
                                            strqry = strqry & "	U_Z_HR_JobstCode,U_Z_HR_JobstName,U_Z_HR_OrgstCode,U_Z_HR_OrgstName,U_Z_HR_SalaryCode,U_Z_SalFrom,U_Z_SalTo  from OHEM where empID='" & strempid & "'"
                                            ...
                                            If otemp.RecordCount > 0 Then
                                                ...
                                                oApplication.Utilities.setEdittextvalue(oForm, "44", otemp1.Fields.Item("Remarks").Value)

                                                oApplication.Utilities.setEdittextvalue(oForm, "1000011", otemp.Fields.Item("dept").Value)
                                                oApplication.Utilities.setEdittextvalue(oForm, "46", otemp.Fields.Item("U_Z_HR_PosiCode").Value)
                                                oApplication.Utilities.setEdittextvalue(oForm, "48", otemp.Fields.Item("U_Z_HR_PosiName").Value)
                                                oApplication.Utilities.setEdittextvalue(oForm, "50", otemp.Fields.Item("U_Z_HR_JobstCode").Value)
                                                oApplication.Utilities.setEdittextvalue(oForm, "52", otemp.Fields.Item("U_Z_HR_JobstName").Value)
                                                oApplication.Utilities.setEdittextvalue(oForm, "54", otemp.Fields.Item("U_Z_HR_OrgstCode").Value)
                                                oApplication.Utilities.setEdittextvalue(oForm, "56", otemp.Fields.Item("U_Z_HR_OrgstName").Value)

                                                oApplication.Utilities.setEdittextvalue(oForm, "90", otemp.Fields.Item("U_Z_SalFrom").Value)
                                                oApplication.Utilities.setEdittextvalue(oForm, "91", otemp.Fields.Item("U_Z_SalTo").Value)
                                            End If
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST
                                ...
                                Try
                                    ...
                                    If (oCFLEvento.BeforeAction = False) Then
                                        ...
                                        'If pVal.ItemUID = "79" Then
                                        '    val = oDataTable.GetValue("U_Z_SalCode", 0)
                                        '    Try
                                        '        oApplication.Utilities.setEdittextvalue(oForm, "79", val)
                                        '    Catch ex As Exception
                                        '    End Try
                                        'End If
